...
実装前に必ず読むこと。要点は4つ。(1) 先頭の「字」を落とし丁目の漢数字を算用数字に直したキーで
系列をまとめる。**同一年に同じキーの表記が2つ来たらその年の変換を失敗させる**（合算しない）。
(2) 表記の例外9件は `data/aza_crosswalk.csv` に人が書く。(3) **区画再編で切れた系列は
つながない**（`succeeded_by` / `split_into` で関係だけ記録）。(4) 宜野湾市の `大山` `野嵩` などは
同名の丁目と**同じ年に併存する別実体**。統合すると二重計上になる（`relation=separate`）。
...
- 字の名寄せは方針決定済み・**未実装**（Phase 3）。`data/aza_crosswalk.csv` はまだ空
- 字別人口に原因未確認の異常が1つ。**2019→2020 で県計が +1.57%**（他の年は ±0.5% 程度）、
  しかも全市町村で一様。県公表の市町村計と突き合わせるまで注記の要否を決めない
